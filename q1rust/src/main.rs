...
struct BTreeNode<'a> {
    ...
    x: f32, y: f32, // Posição x,y do vértice (para quesito de representação gráfica)
}
...
fn calculate_positions(tree: BTreeNode, level: i32, scale: f32, left_limit: f32) -> (BTreeNode, f32) {
    ...
    match tree {
        ...
        BTreeNode {left: None, right: None, ..} => {
            let x = left_limit;
            (
                BTreeNode {
                    x: x, y: y, 
        ...
        BTreeNode {left: Some(left_child), right: None, ..} => {
            // calcula a posição do filho esquerdo
            let (left_child, right_limit) = calculate_positions(*left_child, level + 1, scale, left_limit);
        ...
        BTreeNode {left: None, right: Some(right_child), ..} => {
            // mesma lógica da função acima
            let (right_child, right_limit) = calculate_positions(*right_child, level + 1, scale, left_limit);
            (
                BTreeNode {
                    x: right_child.x, y: y, 
        ...
        BTreeNode {left: Some(left_child), right: Some(right_child), ..} => {
            // calcula as posições dos dois filhos
            let (left_child, lchild_right_limit) = calculate_positions(*left_child, level + 1, scale, left_limit);
            let (right_child, rchild_right_limit) = calculate_positions(*right_child, level + 1, scale, lchild_right_limit + scale);
            // o nó vai estar no meio dos dois filhos
            (
                BTreeNode {
                   x: (left_child.x + right_child.x) / 2.0, y: y,
                   left: Some(Box::new(left_child)), right: Some(Box::new(right_child)),
                   ..tree
                }, 
                rchild_right_limit
            )
...
fn print_tree(node: &BTreeNode, level: usize) {
    ...
    println!("{}{} ({}), x: {}, y: {}", "  ".repeat(level), node.id, node.val, node.x, node.y);
    
    if let Some(left) = &node.left { print_tree(left, level + 1); }
    if let Some(right) = &node.right { print_tree(right, level + 1); }
}
...
fn main() {
    ...
    while !rl.window_should_close() {
        // controle da câmera
        if rl.is_key_down(KeyboardKey::KEY_UP) { camera.target.y -= 2.0; }
        if rl.is_key_down(KeyboardKey::KEY_DOWN) { camera.target.y += 2.0; }
        if rl.is_key_down(KeyboardKey::KEY_RIGHT) { camera.target.x += 2.0; }
        if rl.is_key_down(KeyboardKey::KEY_LEFT) { camera.target.x -= 2.0; }
...
fn draw_tree(tree: &BTreeNode, dc: &mut RaylibMode2D<'_, RaylibDrawHandle<'_>>) {
    ...
    if let Some(left_child) = &tree.left {
        dc.draw_line(tree.x as i32, tree.y as i32, left_child.x as i32, left_child.y as i32, Color::WHITE);
        draw_tree(&left_child, dc);
    }
    if let Some(right_child) = &tree.right {
        dc.draw_line(tree.x as i32, tree.y as i32, right_child.x as i32, right_child.y as i32, Color::WHITE);
    ...
    dc.draw_text(tree.id, tree.x as i32 + 2, tree.y as i32 + 2, 10, Color::WHITE);
    dc.draw_text(&tree.val.to_string(), tree.x as i32 - 4, tree.y as i32 - 6, 12, Color::WHITE);
}